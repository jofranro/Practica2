...
await client.connect(
    "mongodb+srv://jofranro:jofranro@cluster0.iczdkf2.mongodb.net/?authMechanism=SCRAM-SHA-1",
);
const db = client.database("jofranro");
...
.post("/addCar", async (context) => {
    ...
    const value = await result.value; //value es el valor que se le pasa por el body y que se va a guardar en la base de datos
    //comprueba que no existe ya un coche con la misma matrícula
    if(await db.collection<CocheSchema>("coches").findOne({matricula:value.matricula})){    //Ahora no se repite si es con la misma matricula, da error 400 y ya esta
